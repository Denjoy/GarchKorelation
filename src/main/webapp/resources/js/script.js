...
var wall_colors_1x1 = ['#01243B', '#336699', '#5288DB']; //, '#9DA7B2', '#C5C5C5'
var wall_ideas = [];
var wall_squares = [];
wall_squares[0] = [];
var wall_cols_count;
...
$(function() {
    ...
    /*
     * Set minimal width for content area (because footer must be in the very bottom of screen):
     */
    stickFooterOnLoad();
...
var addIdea = function(idea, row, col){
    ...
    var e = '<a class="wall-idea type'+idea.type+'" style="left: '+left+'px; top: '+top+'px; '+color+' " ><div class="wall-idea-wrapper"><div class="wall-idea-img" ';
    if(idea.images[0]!=undefined) {
    	e+='style="background: url('+idea.images[0].url+');"';
    }
    e+='></div><div class="wall-idea-details" ><div class="wall-idea-details-wrapper"><h3>'+idea.title+'</h3><div class="date">'+idea.date+' by '+idea.author+'</div>'+idea.intro+'</div></div></div></a>';
    var $el = $(e);
...
var wallInit = function(){
    ...
    // Formula of counting: wall_block_width = (wall_width - wall_padding*(wall_cols_count-1)) / wall_cols_count
    if (wall_width >= 1140){
        wall_cols_count = 6;
        ...
        while (!$finished){
            ...
            for (col = 0; col<wall_cols_count; col++){
                if (typeof wall_squares[row][col] == 'undefined'){
                    switch (idea.type){
                        case "2x2":
                            if (wall_cols_count == 1 || (col<wall_cols_count-1 && typeof wall_squares[row+1][col] == 'undefined' && typeof wall_squares[row][col+1] == 'undefined' && typeof wall_squares[row+1][col+1] == 'undefined')){
                            ...
                            break;
                        case "2x1":
                            if (wall_cols_count == 1 || (col<wall_cols_count-1 && typeof wall_squares[row][col+1] == 'undefined')){
                                addIdea(idea, row, col);
                                wall_squares[row][col] = true;
                                wall_squares[row][col+1] = true;
                                $finished = true;
            ...
            if (row > 2 * (wall_ideas.length + ideas.length)) break; // Делаем проверку на всякий случай, в качестве защиты от зацикливания
        }
        if (row >= wall_rows_count) wall_rows_count = row;
    });
    $("#wall").height(wall_block_width * wall_rows_count + wall_padding * (wall_rows_count-1));
}
...
var like = function(idea_id){
    $(".idea-like[data-idea-id="+idea_id+"]").removeClass('idea-like-ready').addClass('idea-like-progress')
    $.post({
        url: like_url,
        data: {
            "idea_id": idea_id
    ...
    .done(function( count ) {
        ...
        $like_btn.removeClass('idea-like-progress').addClass('idea-like-done').find("span").text(count);
...
var likeCancel = function(idea_id){
    $(".idea-like[data-idea-id="+idea_id+"]").removeClass('idea-like-done').addClass('idea-like-progress')
    $.post({
        url: likecancel_url,
        data: {
            idea_id: idea_id
    ...
    .done(function() {
        var $like_btn = $(".idea-like[data-idea-id="+idea_id+"]");
        $like_btn.removeClass('idea-like-progress').addClass('idea-like-ready').find("span").text('Like');
        $like_btn.off('click').on('click', function(){
...
var favorite = function(idea_id){
    $(".idea-favorite[data-idea-id="+idea_id+"]").removeClass('idea-favorite-ready').addClass('idea-favorite-progress')
    $.post({
        url: favorite_url,
        data: {
            idea_id: idea_id
    ...
    .done(function( count ) {
        ...
        $favorite_btn.removeClass('idea-favorite-progress').addClass('idea-favorite-done').find("span").text(count);
...
var favoriteCancel = function(idea_id){
    $(".idea-favorite[data-idea-id="+idea_id+"]").removeClass('idea-favorite-done').addClass('idea-favorite-progress')
    $.post({
    ...
    .done(function() {
        var $favorite_btn = $(".idea-favorite[data-idea-id="+idea_id+"]");
        $favorite_btn.removeClass('idea-favorite-progress').addClass('idea-favorite-ready').find("span").text('Favourite');
        $favorite_btn.off('click').on('click', function(){
...
$.fn.magnify = function(options) {
    this.addClass('magnify').append('<div class="magnify-square"></div><div class="magnify-icon"><i class="fa fa-search fa-'+options.size+'"></i></div>');
    return this;
};

/*
 * Start all events, handlers, animations, etc. for idea container after its adding
 */
...
var previewsBeSquare = function(){
    ...
    $('.idea-slider-arrow').css('bottom', $('.idea-slider-item').width()/2 - 12);
}
...
var commentEventsBind = function(){
    $('.comment-reply').on('click', function(){
        ...
        if ($comment_links.siblings('.comment-form').length == 0){
            $('.comment-form.example').clone().removeClass('example').hide().insertAfter($comment_links).slideDown();
        }else{
            var $comment_form = $comment_links.siblings('.comment-form');
...
var stickFooter = function(){
    if ($(".content .container").length == 1){
        var content_min_height = $(window).height() - $(".header").height() - $(".footer").height();
...
var addIdeaImageBindEvents = function($el){
    if ($($el).val() != "" && $($el).siblings("input.add-idea-image").length < add_idea_images_limit-1){
        
        $('.add-idea-images-wrapper').append('<input type="file" class="form-control add-idea-image" name="image[]" /><br />');
        $(".add-idea-image").on('change', function(){
            addIdeaImageBindEvents($(this));
        })
    }
